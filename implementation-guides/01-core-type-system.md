...
export function getConfigurationDisplayName(config: SimulationConfiguration): string {
  ...
  const integrator = config.integrator?.charAt(0).toUpperCase() + config.integrator?.slice(1);
  const algorithm = config.algorithm?.split('-').map(word => 
...
export function getConfigurationShortName(config: SimulationConfiguration): string {
  ...
  const integratorShort = config.integrator?.charAt(0).toUpperCase() + 
                         config.integrator?.slice(1, 3);
...
describe('SimulationConfiguration', () => {
  ...
  describe('display names', () => {
    it('should generate correct display names', () => {
      expect(getConfigurationDisplayName({ mode: "ideal" }))
        .toBe("Ideal Orrery");
      
      expect(getConfigurationDisplayName({
        mode: "nbody",
        integrator: "verlet",
        algorithm: "barnes-hut"
    ...
    it('should generate correct short names', () => {
      expect(getConfigurationShortName({ mode: "ideal" }))
        .toBe("Ideal");
      
      expect(getConfigurationShortName({
        mode: "nbody",
        integrator: "verlet",
        algorithm: "fmm"
      })).toBe("FMM-Ver");
