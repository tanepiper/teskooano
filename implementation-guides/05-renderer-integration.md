...
export class RendererStateAdapter extends StateSubscriptionMixin {
  ...
  constructor() {
    ...
    this.$visualSettings = new BehaviorSubject<RendererVisualSettings>({
      trailLengthMultiplier: initialSimState.visualSettings.trailLengthMultiplier,
      physicsEngine: this.mapConfigurationToPhysicsEngine(initialSimState.simulationConfig),
   ...
   */
  private mapConfigurationToPhysicsEngine(config: SimulationConfiguration): "keplerian" | "verlet" {
  ...
  private extractVisualSettings(simState: SimulationState): RendererVisualSettings {
    return {
      ...
      physicsEngine: this.mapConfigurationToPhysicsEngine(simState.simulationConfig),
...
**Implementation:**
```typescript
import type { SimulationMode, AlgorithmType, IntegratorType } from "@teskooano/core-state";
...
export class OrbitsManager {
   ...
   */
  private handleModeTransition(fromMode: SimulationMode, toMode: SimulationMode): void {
    if (this.modeTransitionInProgress) return;
    
    this.modeTransitionInProgress = true;
    
    console.log(`[OrbitsManager] Transitioning from ${fromMode} to ${toMode} mode`);
   ...
   */
  private createAnalyticalOrbitPath(objectId: string, elements: OrbitalElements): void {
   ...
   */
  private calculateKeplerianPosition(elements: OrbitalElements, meanAnomaly: number): {x: number, y: number, z: number} {
...
export class PredictionManager {
  ...
  private generateAnalyticalPrediction(
    ...
    for (const [id, body] of Object.entries(bodies)) {
      if (body.orbitalElements && body.parentId) {
        predictions[id] = this.calculateAnalyticalOrbitPath(body.orbitalElements, steps, duration);
      } else {
        // Stationary bodies in ideal mode
        predictions[id] = Array(steps).fill(new Vector3(body.position.x, body.position.y, body.position.z));
      }
    }
  ...
  private generatePhysicsPrediction(
    ...
    return this.generatePhysicsBasedPrediction(bodies, adjustedSteps, duration, {
   ...
   */
  private adjustStepsForAlgorithm(baseSteps: number, algorithm?: AlgorithmType): number {
    switch (algorithm) {
      case "direct":
