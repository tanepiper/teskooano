      ...
      request.onsuccess = (event) => {
        ...
        countRequest.onsuccess = () => {
          ...
          getAllRequest.onerror = (event) => {
            console.error(
              "[TextureCache] Error getting records:",
        ...
        countRequest.onerror = (event) => {
          console.error(
            "[TextureCache] Error counting records:",
            (event.target as IDBRequest).error,
          );
          reject((event.target as IDBRequest).error);
        };
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        clearRequest.onerror = (event) => {
          console.error(
            "[TextureCache] Error clearing cache:",
            (event.target as IDBRequest).error,
          );
          reject((event.target as IDBRequest).error);
        };
...
export class SettingsPanel implements IContentRenderer {
  ...
  init(params: IDockviewPanelProps<any>): void {
    ...
    this.cardElement = document.createElement(
      "teskooano-card",
    ) as TeskooanoCard;
    this.formElement = document.createElement(
      "teskooano-form",
    ) as TeskooanoForm;
    ...
    // --- Trail Length Slider ---
    this.trailSliderElement = document.createElement(
      "teskooano-slider",
    ) as TeskooanoSlider;
    ...
    this.trailSliderElement.setAttribute(
      "help-text",
      "This controls the length of the orbit trail. Higher values mean longer trails.",
    );
    ...
    this.engineSelectElement = document.createElement(
      "teskooano-select",
    ) as TeskooanoSelect;
    this.engineSelectElement.id = "setting-physics-engine";
    this.engineSelectElement.setAttribute("label", "Physics Engine");
    this.engineSelectElement.setAttribute(
      "help-text",
      "Select the physics engine to use for the simulation.",
    );
    // Populate options
    ...
    this.cacheStatsElement.style.backgroundColor =
      "var(--color-background-light, #22223a)";
    ...
    this.cacheClearButton = document.createElement(
      "teskooano-button",
    ) as TeskooanoButton;
  ...
  private async updateCacheStats(): Promise<void> {
    ...
    try {
      ...
      if (stats) {
        ...
        this.cacheStatsElement.textContent =
          "Unable to retrieve texture cache information.";
        if (this.cacheClearButton) {
          this.cacheClearButton.setAttribute("disabled", "true");
        }
      }
      ...
      if (this.cacheStatsElement) {
        this.cacheStatsElement.textContent =
          "Error retrieving texture cache information.";
      }
      if (this.cacheClearButton) {
        this.cacheClearButton.setAttribute("disabled", "true");
      }
