...
export class TeskooanoLabeledValue extends HTMLElement {
  ...
  constructor() {
    ...
    this.valueSpan = this.shadowRoot!.querySelector('.value')!;
     ...
     this.labelSlot.addEventListener('slotchange', () => this.updateAttribute('label', this.getAttribute('label')));
     this.valueSlot.addEventListener('slotchange', () => this.updateAttribute('value', this.getAttribute('value')));
  }
  ...
  private updateAttribute(name: string, value: string | null) {
    switch (name) {
      case 'label':
        ...
        } else if (value === null && this.labelSlot.assignedNodes().length === 0) {
      ...
      case 'value':
        ...
        } else if (value === null && this.valueSlot.assignedNodes().length === 0) {
             // Clear the span if attribute removed and slot empty
