import type { DockviewApi, IDockviewPanel, IContentRenderer, GroupPanelPartInitParameters } from "dockview-core";
...
class ModalContentRenderer implements IContentRenderer {
  ...
  constructor() {
    ...
    try {
      ...
      console.log("ModalContentRenderer: TeskooanoModal element styled and appended.");
    } catch (error) {
       console.error("ModalContentRenderer: Error during constructor element styling/appending:", error);
  ...
  init(parameters: GroupPanelPartInitParameters): void {
    console.log("ModalContentRenderer: Init called with params:", parameters.params);
    try {
...
export class ModalManager {
  ...
  private _activeModals: Map<string, {
   ...
   */
  constructor(dockviewController: DockviewController, dockviewContainer: HTMLElement) {
  ...
  public show(options: ModalOptions): Promise<ModalResult> {
    return new Promise((resolve, reject) => {
      const panelId = options.id || `modal-panel-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`;
      ...
      console.log(`ModalManager: Container Rect - width: ${containerRect.width}, height: ${containerRect.height}, scrollTop: ${containerScrollTop}, scrollLeft: ${containerScrollLeft}`);
      console.log(`ModalManager: Calculated position (relative to container scroll origin) - top: ${top}, left: ${left}`);
      ...
      setTimeout(() => {
          ...
          try {
              ...
              if (group && group.element) { // Check if group and its element exist
                  console.log("ModalManager: Attempting querySelector on group's element (delayed)");
                  modalElement = group.element.querySelector('teskooano-modal') as TeskooanoModal;
                  if (modalElement) {
                      console.log("ModalManager: Found modalElement via group.element.querySelector");
                  }
              } else {
                  console.log("ModalManager: panelApi.group or group.element not found.");
              }
          } catch (e) {
          ...
          if (!modalElement) {
              ...
              if(panelElement) {
                 console.log("ModalManager: Attempting querySelector on panel.element as fallback (delayed)");
                 modalElement = panelElement.querySelector('teskooano-modal') as TeskooanoModal;
                 if (modalElement) {
                      console.log("ModalManager: Found modalElement via panel.element.querySelector");
                 }
              } else {
                 console.log("ModalManager: panelApi.element not found for fallback querySelector.");
              }
          }
          ...
          if (!modalElement) {
              ...
              if (panelContent && typeof panelContent.modalElement === 'object' && panelContent.modalElement instanceof HTMLElement) {
                 console.log("ModalManager: Found modalElement via panel.content.modalElement (fallback)");
          ...
          if (modalElement) {
             console.log("ModalManager: Modal element obtained. Setting up interaction.");
             this.setupModalInteraction(panelId, panel, modalElement, resolve, reject);
          } else {
             console.error(`ModalManager: Could not find teskooano-modal within panel ${panelId} after delay (checked group/panel elements and content).`);
  ...
  private _resolveModal(panelId: string, result: ModalResult): void {
    ...
    console.log(`ModalManager: Resolving modal ${panelId} with result: ${result}`);
  ...
  public dispose(): void {
    this._activeModals.forEach((modalData, panelId) => {
      console.warn(`ModalManager: Closing unresolved modal ${panelId} during dispose.`);
      this._resolveModal(panelId, 'dismissed');
    });
