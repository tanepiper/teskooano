...
/**
 * Renders UI controls specifically for Hypergiant stars.
 * Focuses on their unique properties like extreme luminosity and instability.
 */
export class HypergiantUniformsRenderer extends BaseUniformsRenderer {
  public render(
    ...
    this._createHypergiantEffectsSection(container, celestial, subscriptions);
  ...
  private _createBasicPropertiesSection(
    ...
    basicHeader.textContent = "Hypergiant Colors";
    ...
    // Hot Color - for extreme luminosity
    const hotColorControl = this._createColorInput(
      "Hot Color (Extreme Luminosity):",
      celestial.id,
      celestial,
      ["hotColor"],
    );
    ...
    // Cool Color - for instability regions
    const coolColorControl = this._createColorInput(
      "Cool Color (Instability):",
      celestial.id,
      celestial,
      ["coolColor"],
    );
  ...
  private _createHypergiantEffectsSection(
    container: HTMLElement,
    celestial: CelestialObject,
    subscriptions: Subscription[],
  ): void {
    const effectsHeader = document.createElement("h3");
    effectsHeader.textContent = "Hypergiant Effects";
