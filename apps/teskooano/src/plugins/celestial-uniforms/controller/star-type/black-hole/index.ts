...
/**
 * Renders UI controls specifically for Black Holes.
 * Focuses on their unique properties like gravitational lensing and accretion disks.
 */
export class BlackHoleUniformsRenderer extends BaseUniformsRenderer {
  public render(
    ...
    this._createBlackHoleEffectsSection(container, celestial, subscriptions);
  ...
  private _createBasicPropertiesSection(
    ...
    basicHeader.textContent = "Black Hole Colors";
    ...
    // Hot Color - for accretion disk
    const hotColorControl = this._createColorInput(
      "Hot Color (Accretion Disk):",
      celestial.id,
      celestial,
      ["hotColor"],
    );
    ...
    // Cool Color - for gravitational lensing
  ...
  private _createBlackHoleEffectsSection(
    container: HTMLElement,
    celestial: CelestialObject,
    subscriptions: Subscription[],
  ): void {
    const effectsHeader = document.createElement("h3");
    effectsHeader.textContent = "Black Hole Effects";
