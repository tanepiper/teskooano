...
/**
 * Renders UI controls specifically for Protostars.
 * Focuses on their unique properties like accretion disks and formation processes.
 */
export class ProtostarUniformsRenderer extends BaseUniformsRenderer {
  public render(
    ...
    this._createProtostarEffectsSection(container, celestial, subscriptions);
  ...
  private _createBasicPropertiesSection(
    ...
    basicHeader.textContent = "Protostar Colors";
    ...
    // Hot Color - for accretion hotspots
    const hotColorControl = this._createColorInput(
      "Hot Color (Accretion Hotspots):",
      celestial.id,
      celestial,
      ["hotColor"],
    );
    ...
    // Cool Color - for formation regions
    const coolColorControl = this._createColorInput(
      "Cool Color (Formation):",
      celestial.id,
      celestial,
      ["coolColor"],
    );
  ...
  private _createProtostarEffectsSection(
    container: HTMLElement,
    celestial: CelestialObject,
    subscriptions: Subscription[],
  ): void {
    const effectsHeader = document.createElement("h3");
    effectsHeader.textContent = "Protostar Effects";
