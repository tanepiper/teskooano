...
/**
 * Renders UI controls specifically for Wolf-Rayet stars.
 * Focuses on their unique properties like strong stellar winds and high mass loss.
 */
export class WolfRayetUniformsRenderer extends BaseUniformsRenderer {
  public render(
    ...
    this._createWolfRayetEffectsSection(container, celestial, subscriptions);
  ...
  private _createBasicPropertiesSection(
    ...
    basicHeader.textContent = "Wolf-Rayet Colors";
    ...
    // Hot Color - for stellar winds
    const hotColorControl = this._createColorInput(
      "Hot Color (Stellar Winds):",
      celestial.id,
      celestial,
      ["hotColor"],
    );
    ...
    // Cool Color - for wind interaction
    const coolColorControl = this._createColorInput(
      "Cool Color (Wind Interaction):",
      celestial.id,
      celestial,
      ["coolColor"],
    );
  ...
  private _createWolfRayetEffectsSection(
    container: HTMLElement,
    celestial: CelestialObject,
    subscriptions: Subscription[],
  ): void {
    const effectsHeader = document.createElement("h3");
    effectsHeader.textContent = "Wolf-Rayet Effects";
