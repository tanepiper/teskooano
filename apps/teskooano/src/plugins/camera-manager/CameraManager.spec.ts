...
describe('CameraManager', () => {
  ...
  it('should initialize with default values if none provided', () => {
    ...
    expect(state.currentPosition.equals(new THREE.Vector3(200, 200, 200))).toBe(true); // DEFAULT_CAMERA_POSITION
   ...
   it('should initialize with provided options', () => {
    ...
    renderableObjectsStore.set({
      [initialFocusId]: createMockRenderable(initialFocusId, initialTarget) as any
    });
  ...
  it('initializeCameraPosition should set renderer camera and controls target', () => {
     ...
     expect(mockRenderer.controlsManager.controls.target.equals(initialTarget)).toBe(true);
  ...
  it('focusOnObject should call renderer methods and update internal state intent', () => {
    ...
    expect(cameraManager.getCameraStateAtom().getValue().focusedObjectId).toBe(objectId);

    // Check renderer methods are called
    expect(mockRenderer.setFollowTarget).toHaveBeenCalledWith(objectId, expect.any(THREE.Vector3), expect.any(THREE.Vector3));
    ...
    expect(mockRenderer.setFollowTarget).toHaveBeenCalledWith(objectId, expectedTarget, expectedPosition);
  ...
  it('focusOnObject(null) should clear focus and move to default', () => {
    // First focus on something
     ...
     cameraManager.focusOnObject(objectId);
     expect(cameraManager.getCameraStateAtom().getValue().focusedObjectId).toBe(objectId); // Verify initial focus

     // Now clear focus
     cameraManager.focusOnObject(null);

     expect(cameraManager.getCameraStateAtom().getValue().focusedObjectId).toBeNull(); // Intent updated
   ...
   it('focusOnObject should handle missing object', () => {
     ...
     expect(cameraManager.getCameraStateAtom().getValue().focusedObjectId).toBeNull();
  ...
  it('pointCameraAt should call controlsManager method', () => {
    ...
    expect(mockRenderer.controlsManager.pointCameraAtTarget).toHaveBeenCalledWith(
  ...
  it('handleCameraTransitionComplete should update state from event detail', () => {
   ...
   it('handleCameraTransitionComplete should update state position/target from event without focus id', () => {
    ...
    cameraManager.getCameraStateAtom().next({ ...initialVal, focusedObjectId: 'initialFocus' });
