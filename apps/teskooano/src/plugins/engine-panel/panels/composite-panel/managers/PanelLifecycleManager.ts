...
export class PanelLifecycleManager {
	...
	constructor(options: PanelLifecycleManagerOptions) {
		...
		this._handleSystemGenerationStart = this._handleSystemGenerationStart.bind(this);
		this._handleSystemGenerationComplete = this._handleSystemGenerationComplete.bind(this);
	}
	...
	public listen(): void {
				...
				if (hasObjects) {
					...
					if (rendererExists) {
						...
						this._clearTimeout = window.setTimeout(() => {
							console.log("[PanelLifecycleManager] Disposing renderer after delay");
		...
		// Subscribe to RxJS engine signals instead of DOM events
		this._subscription.add(
			engineSignalsService.systemGenerationStart$.subscribe(
				this._handleSystemGenerationStart,
			),
		);
		this._subscription.add(
			engineSignalsService.systemGenerationComplete$.subscribe(
