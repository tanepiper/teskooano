...
sharedTemplate.innerHTML = `
  <style>
    ...
    .capabilities-table, .optimization-table, .device-table {
      width: 100%;
      border-collapse: collapse;
      font-size: 0.85em;
    }
    
    .capabilities-table td, .optimization-table td, .device-table td {
      padding: 2px 4px;
    ...
    .performance-value.high-end, .performance-value.excellent {
      background: var(--color-success, #4caf50);
      color: #fff;
    }
    
    .performance-value.mid-range, .performance-value.good {
      background: var(--color-warning, #ff9800);
      color: #fff;
    }
    
    .performance-value.low-end, .performance-value.fair, .performance-value.limited {
      background: var(--color-error, #f44336);
      color: #fff;
    }
    
    .performance-value.cosmic {
