...
export const CelestialInfoPanel = defineComponent({
  ...
  lifecycle: {
    mounted() {
      document.addEventListener('renderer-focus-changed', this.handleFocusChange);
    },
    
    unmounted() {
      document.removeEventListener('renderer-focus-changed', this.handleFocusChange);
    }
  },
...
export function defineComponent(definition: ComponentDefinition): ComponentConstructor {
  return class extends HTMLElement implements IContentRenderer {
    ...
    private initializeState() {
      this._state = new Proxy({ ...definition.state }, {
    ...
    private setupSubscriptions() {
      definition.subscriptions?.forEach(sub => {
        ...
        const handler = typeof sub.handler === 'string' 
    ...
    private resolveObservable(source: string | Observable<any>): Observable<any> {
...
export const CelestialInfoPanel = defineComponent({
  ...
  computed: {
    typeClass: {
      deps: ['selectedObject'],
      compute(selectedObject) {
        return selectedObject ? `type-${selectedObject.type.toLowerCase()}` : '';
      }
    },
  ...
  methods: {
    ...
    executeAction(actionId) {
      this.emit('action-executed', { actionId, object: this.state.selectedObject });
    }
  }
});
```
...
export function createPlugin(definition: PluginDefinition): TeskooanoPlugin {
  ...
  const processedPanels = processPanels(definition.panels, discoveredComponents);
  ...
  return {
    ...
    functions: Object.entries(definition.functions || {}).map(([id, config]) => ({
      id: `${definition.id}:${id}`,
      ...config
    })),
...
function discoverComponents(pattern?: string | ComponentMap): ComponentConfig[] {
...
function processPanels(panels?: PanelMap, components: ComponentConfig[]) {
  ...
  Object.entries(panels).forEach(([panelId, config]) => {
    ...
    if (!component) {
      throw new Error(`Component ${componentName} not found for panel ${panelId}`);
    }
    ...
    if (config.toolbar) {
      ...
      const registration = toolbarRegistrations.find(tr => tr.target === config.toolbar)!;
...
export class TemplateBinding {
  ...
  private findAttributeBindings() {
    const elements = this.element.querySelectorAll('[\\:class], [\\:disabled], [\\:data]');
  ...
  private updateTextBinding(config: TextBindingConfig) {
    ...
    const newText = config.originalText.replace(/\{\{([^}]+)\}\}/g, (match, expr) => {
