...
export async function requestDisplayMedia(
  ...
  if (!isScreenCaptureSupported()) {
    return Promise.reject(new Error('Screen Capture API (getDisplayMedia) is not supported.'));
  }
  ...
  try {
    ...
    stream.getTracks().forEach(track => {
        track.onended = () => {
            console.log(`Screen capture track (${track.kind}: ${track.label || 'N/A'}) ended.`);
...
export function requestDisplayMediaObservable(
  ...
  return from(requestDisplayMedia(options)).pipe(
    tap(stream => console.log('Display media stream obtained via Observable.')),
    catchError(err => {
      console.error('Error obtaining display media stream via Observable:', err);
