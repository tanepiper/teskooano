...
declare global {
  ...
  // Define the CommandEvent if not already globally available
   ...
   interface Window {
     ...
     CommandEvent?: { new (type: 'command', eventInitDict?: EventInit & { command?: string }): CommandEvent; prototype: CommandEvent; };
   }
}
...
export function createCommandObservable(
  ...
  if (!isInvokerCommandsSupported()) {
    ...
    console.warn('Invoker Commands API prototype properties not found, but attempting to listen for "command" event.');
    // Allow proceeding but with a warning.
  }

  const commandEvent$ = fromEvent<CommandEvent>(targetElement, 'command').pipe(
    filter(event => commandName === undefined || event.command === commandName),
    share()
  );
   ...
   options: EventInit = { bubbles: true, cancelable: true }
    ...
    try {
       ...
       if (typeof window.CommandEvent === 'undefined') {
           console.error('CommandEvent constructor is not supported in this browser.');
           return false;
       }
        ...
        try {
          Object.defineProperty(commandEvent, 'command', { value: command, writable: false });
        } catch (e) {
           console.warn("Could not define 'command' property directly on event. Using detail object.");
        }
        ...
        try {
            console.log(`Falling back to dispatching CustomEvent for command "${command}"`);
            ...
            console.error(`Fallback CustomEvent dispatch failed for command "${command}":`, fallbackErr);
            return false;
        }
    }
}
