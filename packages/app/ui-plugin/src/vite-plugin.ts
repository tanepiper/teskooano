    ...
    async load(id) {
      if (id === RESOLVED_VIRTUAL_MODULE_ID) {
        ...
        try {
          ...
          console.log(
            `  - Found ${Object.keys(componentConfig).length} components in config.`,
          );
          ...
          console.log(
            `  - Found ${Object.keys(pluginConfig).length} plugins in config.`,
          );
          ...
          for (const [tag, config] of Object.entries(componentConfig)) {
            ...
            const resolvedCompPath = path
              ...
              .replace(/\\/g, "/");
            console.log(
              `    - Generating component loader for '${tag}': import('${resolvedCompPath}')`,
            );
          ...
          for (const [id, config] of Object.entries(pluginConfig)) {
            ...
            const resolvedPluginPath = path
              ...
              .replace(/\\/g, "/");
            console.log(
              `    - Generating plugin loader for '${id}': import('${resolvedPluginPath}')`,
            );
          ...
          console.log(
            `[Teskooano UI Plugin] Generated virtual module content.`,
          );
          return content;
        } catch (error) {
          console.error(
