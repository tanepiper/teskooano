...
export const getPhysicsBodies = (): PhysicsStateReal[] => {
  ...
  Object.values(celestialObjectsStore.get()).forEach(obj => {
    if (obj.physicsStateReal) {
      ...
      console.warn(`Object ${obj.id} missing physicsStateReal, skipping in simulation.`);
    }
  });
  return bodies;
};
 ...
 */
export const updatePhysicsState = (updatedBodiesReal: PhysicsStateReal[]): void => {
  ...
  updatedBodiesReal.forEach(bodyReal => {
    ...
    if (!existingObject) {
      console.warn(`[updatePhysicsState] Cannot find existing object with ID: ${id} (for state update)`);
      return; 
    }
    
    // --- Calculate Rotation Quaternion --- 
    let rotationQuaternion = new THREE.Quaternion(); // Initialize to identity
    if (existingObject.siderealRotationPeriod_s && existingObject.siderealRotationPeriod_s !== 0 && existingObject.axialTilt) {
      const currentTime = simulationState.get().time; 
      const rotationSpeed = (2 * Math.PI) / existingObject.siderealRotationPeriod_s;
      const currentRotationAngle = (currentTime * rotationSpeed) % (2 * Math.PI);
      ...
      rotationQuaternion = new THREE.Quaternion().setFromAxisAngle(tiltAxisTHREE, currentRotationAngle);
    } 
    // --- End Rotation Calculation ---
    
    // --- Calculate Scaled Position --- 
    const updatedPositionScaled = bodyReal.position_m.clone()
