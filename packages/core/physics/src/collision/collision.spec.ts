...
import { detectSphereCollision, resolveCollision, handleCollisions } from './collision';
...
describe('Collision Detection (detectSphereCollision)', () => {
  it('should detect collision when spheres overlap', () => {
    const body1 = createRealState('1', { x: 0, y: 0, z: 0 }, { x: 0, y: 0, z: 0 }, 1);
    const body2 = createRealState('2', { x: 2, y: 0, z: 0 }, { x: 0, y: 0, z: 0 }, 1);
    const radius1 = 1.5;
    const radius2 = 1.5;
    ...
    if (collision) {
      ...
      const expectedPoint = body2.position_m.clone().add(collision.normal.clone().multiplyScalar(radius2));
  ...
  it('should not detect collision when spheres touch exactly', () => {
    const body1 = createRealState('1', { x: 0, y: 0, z: 0 }, { x: 0, y: 0, z: 0 }, 1);
    const body2 = createRealState('2', { x: 3, y: 0, z: 0 }, { x: 0, y: 0, z: 0 }, 1);
    const radius1 = 1.5;
    const radius2 = 1.5;
    const collision = detectSphereCollision(body1, radius1, body2, radius2);
  ...
  it('should not detect collision when spheres are separate', () => {
    const body1 = createRealState('1', { x: 0, y: 0, z: 0 }, { x: 0, y: 0, z: 0 }, 1);
    const body2 = createRealState('2', { x: 4, y: 0, z: 0 }, { x: 0, y: 0, z: 0 }, 1);
    const radius1 = 1.5;
    const radius2 = 1.5;
    const collision = detectSphereCollision(body1, radius1, body2, radius2);
    expect(collision).toBeNull();
...
describe('Collision Resolution (resolveCollision)', () => {
  it('should resolve head-on collision correctly (equal mass)', () => {
    const body1 = createRealState('1', { x: 0, y: 0, z: 0 }, { x: 1, y: 0, z: 0 }, 1);
    const body2 = createRealState('2', { x: 1, y: 0, z: 0 }, { x: -1, y: 0, z: 0 }, 1);
    ...
    const [newState1, newState2] = resolveCollision(collisionInfo, body1, body2);
  ...
  it('should resolve collision with one stationary body (equal mass)', () => {
    const body1 = createRealState('1', { x: 0, y: 0, z: 0 }, { x: 2, y: 0, z: 0 }, 1);
    const body2 = createRealState('2', { x: 1, y: 0, z: 0 }, { x: 0, y: 0, z: 0 }, 1);
    const radius = 0.6;
    const collisionInfo = detectSphereCollision(body1, radius, body2, radius);
    expect(collisionInfo).not.toBeNull();
    if (!collisionInfo) return;
   ...
   it('should resolve head-on collision correctly (unequal mass)', () => {
    const body1 = createRealState('1', { x: 0, y: 0, z: 0 }, { x: 1, y: 0, z: 0 }, 2); // Mass = 2
    const body2 = createRealState('2', { x: 1, y: 0, z: 0 }, { x: -1, y: 0, z: 0 }, 1); // Mass = 1
    const radius = 0.6;
    const collisionInfo = detectSphereCollision(body1, radius, body2, radius);
    expect(collisionInfo).not.toBeNull();
    if (!collisionInfo) return;
...
describe('Collision Handling (handleCollisions)', () => {
  it('should handle multiple simultaneous collisions', () => {
    const body1 = createRealState('1', { x: 0, y: 0, z: 0 }, { x: 1, y: 0, z: 0 }, 1);
    const body2 = createRealState('2', { x: 1, y: 0, z: 0 }, { x: -1, y: 0, z: 0 }, 1);
    const body3 = createRealState('3', { x: 2, y: 0, z: 0 }, { x: -2, y: 0, z: 0 }, 1);
    const radius = 0.6;
    const radii = new Map<string | number, number>([['1', radius], ['2', radius], ['3', radius]]);
