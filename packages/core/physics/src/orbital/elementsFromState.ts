...
export function calculateElementsFromStateVectors(
  ...
  if (Math.abs(eccentricity - 1.0) < EPSILON) {
    ...
    // Elliptical orbit (including circular)
    semiMajorAxis_m = -mu / (2 * energy);
  }

  if (semiMajorAxis_m < EPSILON && eccentricity < 1.0) {
    // Prevent issues with near-zero SMA for bound orbits
    console.warn(
  ...
  if (n < EPSILON) {
    // Equatorial orbit (n is zero vector)
    ...
    if (nVec.z < 0) {
      // If node vector points towards negative Z in XZ plane
  ...
  if (eccentricity < EPSILON) {
    ...
    if (n < EPSILON) {
      ...
      if (eVec.z < 0) {
        // Check component in the reference plane (Z here)
      ...
      if (eVec.y < 0) {
        // Check component normal to the reference plane (Y here)
        argumentOfPeriapsis = 2 * Math.PI - argumentOfPeriapsis;
      }
    }
  }
  ...
  if (eccentricity < EPSILON) {
    ...
    if (n < EPSILON) {
      ...
      if (rVec.z < 0) {
        // Check component in reference plane (Z here)
      ...
      if (rVec.y < 0) {
        // Check component normal to reference plane (Y here)
        trueAnomaly = 2 * Math.PI - trueAnomaly;
      }
    }
  } else {
    ...
    if (rDotV < 0) {
      // If object is moving towards the central body
      trueAnomaly = 2 * Math.PI - trueAnomaly;
    }
  }
