...
describe('AlgorithmFactory', () => {
  describe('selectOptimalAlgorithm', () => {
    ...
    it('should respect memory constraints', () => {
      ...
      const algorithm = AlgorithmFactory.selectOptimalAlgorithm(10000, { maxMemoryUsage: 'low' });
    ...
    it('should prioritize accuracy when requested', () => {
      const algorithm = AlgorithmFactory.selectOptimalAlgorithm(200, { prioritizeAccuracy: true });
    ...
    it('should select direct for small systems when prioritizing accuracy', () => {
      const algorithm = AlgorithmFactory.selectOptimalAlgorithm(50, { prioritizeAccuracy: true });
      expect(algorithm).toBe('direct'); // Exact accuracy for small systems
    });

    it('should prioritize speed when requested', () => {
      const algorithm = AlgorithmFactory.selectOptimalAlgorithm(5000, { prioritizeSpeed: true });
    ...
    it('should fall back to barnes-hut when no algorithm fits constraints', () => {
      ...
      const algorithm = AlgorithmFactory.selectOptimalAlgorithm(100000, { maxMemoryUsage: 'low' });
  ...
  describe('getPerformanceEstimate', () => {
    ...
    it('should calculate relative speed correctly', () => {
      const estimateSmall = AlgorithmFactory.getPerformanceEstimate('direct', 10);
      const estimateLarge = AlgorithmFactory.getPerformanceEstimate('direct', 1000);
      
      // Smaller body count should have better relative speed for O(N²) algorithm
      expect(estimateSmall.relativeSpeed).toBeGreaterThan(estimateLarge.relativeSpeed);
    });
  });
  ...
  describe('validateAlgorithmChoice', () => {
    ...
    it('should warn about poor algorithm choices', () => {
      const validation = AlgorithmFactory.validateAlgorithmChoice('direct', 2000);
    ...
    it('should recommend better algorithms', () => {
      ...
      expect(validation.recommendations[0]).toContain('might be more efficient');
    });

    it('should warn about specific performance issues', () => {
      const validation = AlgorithmFactory.validateAlgorithmChoice('direct', 600);
      
      expect(validation.warnings.some(w => w.includes('very slow'))).toBe(true);
    });

    it('should warn about FMM overhead for small systems', () => {
      const validation = AlgorithmFactory.validateAlgorithmChoice('fmm', 1500);
      
      expect(validation.warnings.some(w => w.includes('overhead'))).toBe(true);
    });
  });
  ...
  describe('getAlgorithmInfo', () => {
    ...
    it('should return information for all algorithm types', () => {
      const algorithms: AlgorithmType[] = ['direct', 'barnes-hut', 'fmm', 'p3m'];
  ...
  describe('createOptimalConfiguration', () => {
    ...
    it('should respect all preferences', () => {
      const config = AlgorithmFactory.createOptimalConfiguration(2000, 'nbody', {
        prioritizeAccuracy: true,
