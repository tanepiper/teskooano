...
import { threeVectorDebug, isVisualizationEnabled, createLogger } from './index';
...
export class CelestialDebugger {
  ...
  public setVectors(objectId: string, vectors: CelestialVectorPairs): void {
    ...
    if (vectors.lightDirection) {
      threeVectorDebug.setVector(debugName, 'lightDirection', vectors.lightDirection);
    }
    
    if (vectors.parentPosition) {
      threeVectorDebug.setVector(debugName, 'parentPosition', vectors.parentPosition);
    }
    ...
    if (vectors.angularMomentum) {
      threeVectorDebug.setVector(debugName, 'angularMomentum', vectors.angularMomentum);
    }
  }
  ...
  public clearAllCelestialDebugData(): void {
    ...
    try {
      ...
      this.logger.warn('Failed to clear all celestial debug data from localStorage:', e);
    }
  }
}
