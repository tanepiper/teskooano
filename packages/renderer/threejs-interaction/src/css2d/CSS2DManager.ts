...
import { CSS2DRendererService, CSS2DLayerManager, CSS2DLayerType } from "./";
import type {
  ILabelFactory,
...
export class CSS2DManager {
  ...
  render(
    ...
    // Optimized check for visible elements
    if (this.layerManager.hasAnyVisibleElements()) {
      this.rendererService.render(this.scene, this.camera);
    } else {
      // Optionally, if there's any explicit "clear" or "no-op" render call for CSS2DRendererService,
      // it could be called here. Otherwise, just skipping the render is fine.
      // For example, if rendererService accumulates state that needs clearing:
      // this.rendererService.clear(); // (if such a method exists and is needed)
    }
  }
