...
export class ControlsManager {
  ...
  updateTarget(target: THREE.Vector3, withTransition: boolean = true): void {
    ...
    this.startTransition(currentPosition, currentTarget, currentPosition, target);
  }

  /**
   * Move camera to position and target with transition
   */
  moveTo(position: THREE.Vector3, target: THREE.Vector3, withTransition: boolean = true): void {
    if (!withTransition) {
      // Immediate update without transition
   ...
   */
  private calculateTransitionDuration(startPos: THREE.Vector3, endPos: THREE.Vector3): number {
   ...
   */
  private startTransition(startPos: THREE.Vector3, startTarget: THREE.Vector3, 
                         endPos: THREE.Vector3, endTarget: THREE.Vector3): void {
    ...
    const onTimelineComplete = () => {
      ...
      const transitionCompleteEvent = new CustomEvent('camera-transition-complete', {
        detail: {
    ...
    if (rotationDuration > 0.01) { // Only add if duration is meaningful
    ...
    if (positionDuration > 0.01) { // Only add if duration is meaningful
        ...
        }, rotationDuration > 0.01 ? '>' : 0); // Start immediately after previous if it exists, otherwise at time 0
    }
  }
