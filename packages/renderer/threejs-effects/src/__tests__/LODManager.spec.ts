...
describe('LODManager', () => {
  ...
  it('should update LOD based on camera position', () => {
    ...
    const distance = lodMesh.getWorldPosition(new THREE.Vector3()).distanceTo(camera.position);
    ...
    // Calculate the actual distance between camera and LOD mesh
    const distance2 = lodMesh.getWorldPosition(new THREE.Vector3()).distanceTo(camera.position);
    console.log(`[TEST] Distance between camera and LOD mesh (after moving closer): ${distance2}`);
    
    // Check the current LOD level
    const currentLevel2 = lodMesh.getCurrentLevel();
    console.log(`[TEST] Current LOD level (after moving closer): ${currentLevel2}`);
