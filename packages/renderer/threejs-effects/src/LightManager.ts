...
export class LightManager {
  ...
  constructor(scene: THREE.Scene, 
              private objectsStore: MapStore<Record<string, RenderableCelestialObject>> = renderableObjectsStore // Inject store
  ) {
    this.scene = scene;
    ...
    this.unsubscribeStore = this.objectsStore.subscribe(
      (objects: Record<string, RenderableCelestialObject>) => {
        ...
        for (const id in objects) {
          ...
          if (objectData.type === 'STAR' && objectData.position) { // Only update STAR lights with position
   ...
   */
  addStarLight(id: string, position: THREE.Vector3, color: number = 0xffffff, intensity: number = 1.5): void {
  ...
  updateStarLight(id: string, position: THREE.Vector3): void {
    console.warn('[LightManager] updateStarLight is deprecated. Positions are updated via store subscription.');
    const light = this.starLights.get(id);
    if (light) {
      light.position.set(position.x, position.y, position.z);
    }
   ...
   */
  getStarLightsData(): Map<string, { position: THREE.Vector3, color: THREE.Color, intensity: number }> {
    const lightData = new Map<string, { position: THREE.Vector3, color: THREE.Color, intensity: number }>();
    
    this.starLights.forEach((light, id) => {
        // Ensure the light object is a THREE.PointLight or similar with color and intensity
        if (light instanceof THREE.PointLight || light instanceof THREE.SpotLight) { // DirectionalLight doesn't have intensity in the same way
            lightData.set(id, { 
            ...
            console.warn(`LightManager: Light with id ${id} is not a recognized type with color/intensity properties.`);
            lightData.set(id, { 
                position: light.position.clone(), 
