...
export class StateManager {
  ...
  private subscribeToStateChanges(): void {
    ...
    const objUnsubscribe = celestialObjectsStore.subscribe((newObjects, prevObjects) => {
      ...
      for (const id in newObjects) {
        ...
        if (!prevObjects || !prevObjects[id]) {
          ...
          if (currentPhysics && prevPhysics && 
              ...
              const posChanged = currentPhysics.position_m.x !== prevPhysics.position_m.x ||
                                 currentPhysics.position_m.y !== prevPhysics.position_m.y ||
                                 currentPhysics.position_m.z !== prevPhysics.position_m.z;
                                 
              const velChanged = currentPhysics.velocity_mps.x !== prevPhysics.velocity_mps.x ||
          ...
          if (physicsChanged) { // Trigger update if physics state changed
      ...
      if (prevObjects) {
        for (const id in prevObjects) {
          if (!newObjects[id]) {
            // Object was removed
            this.objectSubscribers.forEach(callback => {
              callback('remove', prevObjects[id] as RendererCelestialObject, id);
            });
          }
        }
      }
  ...
  getAllObjects(): Record<string, RendererCelestialObject> {
    return celestialObjectsStore.get() as Record<string, RendererCelestialObject>;
  }
