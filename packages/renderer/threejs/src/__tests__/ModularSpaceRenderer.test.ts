...
import { SceneManager, AnimationLoop, StateManager } from '@teskooano/renderer-threejs-core';
import { ObjectManager, OrbitManager, BackgroundManager } from '@teskooano/renderer-threejs-visualization';
import { ControlsManager, CSS2DManager } from '@teskooano/renderer-threejs-interaction';
...
const createMockCelestialObject = (id: string, type: CelestialType, position: THREE.Vector3) => {
  ...
  const baseObject = {
    ...
    orbitalParameters: type === CelestialType.PLANET ? {
      semiMajorAxis: 10000,
      eccentricity: 0.1,
      inclination: 0,
...
describe('ModularSpaceRenderer Integration Tests', () => {
  ...
  it('should set up event listeners', () => {
    ...
    const gridHelper = renderer.scene.children.find(child => 
  ...
  it('should add and remove objects', () => {
    ...
    const star = createMockCelestialObject('test-star', CelestialType.STAR, starPosition);
    ...
    const planet = createMockCelestialObject('test-planet', CelestialType.PLANET, planetPosition);
    ...
    const orbitLine = renderer.scene.children.find(child => 
  ...
  it('should handle orbital line updates', () => {
    ...
    let orbitLine = renderer.scene.children.find(child => 
      child instanceof THREE.Line && child.name.includes('test-planet')
    );
    ...
    if (!orbitLine) {
      ...
      orbitLine = renderer.scene.children.find(child => 
        child instanceof THREE.Line && child.name.includes('test-planet')
      );
    }
