...
export class RendererStateAdapter {
  ...
  private calculateRotation(axialTilt: OSVector3 | number | undefined, siderealPeriod: number | undefined): THREE.Quaternion {
      ...
      if (axialTilt instanceof OSVector3) {
          ...
          this.tiltQuaternion.setFromAxisAngle(new THREE.Vector3(0, 0, 1), THREE.MathUtils.degToRad(axialTilt));
      }

      // Handle Spin (only if siderealPeriod is provided)
      if (siderealPeriod && siderealPeriod !== 0) {
          const rotationAngle = (this.currentSimulationTime / siderealPeriod) * 2 * Math.PI;
          this.spinQuaternion.setFromAxisAngle(this.rotationAxis, rotationAngle);
          finalRotation.multiplyQuaternions(this.tiltQuaternion, this.spinQuaternion);
      } else {
  ...
  private processStandardObject(
      ...
      const rotation = this.calculateRotation(obj.axialTilt, obj.siderealRotationPeriod_s);
  ...
  private processRingSystem(
      ...
      existing: RenderableCelestialObject | undefined,
      determineLightSource: (id: string) => string | undefined
  ): RenderableCelestialObject | null { // Return null if parent is invalid
      const parentId = obj.parentId;
      if (!parentId) {
          console.warn(`[RendererStateAdapter] Ring system ${obj.id} is missing parentId.`);
          return null;
      }
      const parent = objects[parentId];
      if (!parent || !parent.physicsStateReal || !parent.physicsStateReal.position_m) {
      ...
      const position = physicsToThreeJSPosition(parent.physicsStateReal.position_m);
  ...
  private processCelestialObjectsUpdateNow(objects: Record<string, CelestialObject>): void {
      ...
      try {
          for (const id in objects) {
              ...
              if (!obj.physicsStateReal || !obj.physicsStateReal.position_m) {
                  ...
                  if (obj.type !== CelestialType.RING_SYSTEM) {
                    console.warn(`[RendererStateAdapter] Skipping object ${id} due to missing physics state.`);
                    continue;
                  }
              }
              ...
              switch (obj.type) {
                  case CelestialType.RING_SYSTEM:
                      renderableObject = this.processRingSystem(obj, objects, existing, determineLightSource);
                      break;
                  ...
                  case CelestialType.SPACE_ROCK: // Might need custom logic later
                      renderableObject = this.processStandardObject(obj, existing, determineLightSource);
                      break;
                  default:
                      console.warn(`[RendererStateAdapter] Unhandled celestial type: ${obj.type} for object ${id}`);
              }
          ...
          console.error("[RendererStateAdapter] Error during object processing loop:", error);
      }
  }

  private subscribeToCoreState(): void {
    this.unsubscribeObjects = celestialObjectsStore.subscribe(
    ...
    this.unsubscribeSimState = simulationState.subscribe(
      (simState: SimulationState) => {
        ...
        const newMultiplier = simState.visualSettings.trailLengthMultiplier ?? 30;
        const currentMultiplierInAdapter = this.$visualSettings.get().trailLengthMultiplier;
        // REMOVED Log
        ...
        const newEngineMapped = corePhysicsEngine === 'verlet' ? 'verlet' : 'keplerian';
