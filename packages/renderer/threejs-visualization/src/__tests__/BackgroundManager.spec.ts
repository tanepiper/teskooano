...
describe('BackgroundManager', () => {
  ...
  it('should toggle debug visualization', () => {
    ...
    group.children.forEach((child, index) => {
      console.log(`Child ${index}:`, child.type, child instanceof THREE.AxesHelper ? 'AxesHelper' : '');
      if (child instanceof THREE.Mesh) {
        console.log(`  - Name: ${(child as THREE.Mesh & { name: string }).name || 'unnamed'}`);
        console.log(`  - Material: ${child.material instanceof THREE.Material ? child.material.type : 'unknown'}`);
      }
    });
    
    // Check for debug visuals - look for AxesHelper or any mesh with debug in its name
    const hasDebugVisuals = group.children.some(child => 
      child instanceof THREE.AxesHelper || 
      (child instanceof THREE.Mesh && 
       ((child as THREE.Mesh & { name: string }).name?.includes('debug') || 
        (child.material instanceof THREE.Material && 
         (child.material as THREE.Material & { name?: string }).name?.includes('debug'))))
    );
    
    // If no debug visuals found, check if there are any objects that might be debug-related
    if (!hasDebugVisuals) {
      console.log('No debug visuals found. Checking for any objects that might be debug-related:');
      group.children.forEach((child, index) => {
        if (child instanceof THREE.Mesh) {
          ...
          if (material instanceof THREE.Material) {
            ...
            console.log(`    - Material name: ${(material as THREE.Material & { name?: string }).name || 'unnamed'}`);
          }
        }
      });
    }
