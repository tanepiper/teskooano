...
vi.mock('@teskooano/core-physics', () => ({
  calculateOrbitalPosition: vi.fn().mockImplementation((parentState, orbitalParams, time) => {
...
describe('Orbit Calculator Module', () => {
  ...
  describe('calculateOrbitPoints', () => {
    ...
    it('should use fallback calculation when parent physics state is not available', () => {
      ...
      expect(Math.abs(avgX - mockParentPosition.x)).toBeLessThan(mockOrbitalParameters.semiMajorAxis * mockOrbitalParameters.eccentricity);
      expect(Math.abs(avgY - mockParentPosition.y)).toBeLessThan(1); // Y should be close to parent Y
      expect(Math.abs(avgZ - mockParentPosition.z)).toBeLessThan(mockOrbitalParameters.semiMajorAxis * mockOrbitalParameters.eccentricity);
    });
