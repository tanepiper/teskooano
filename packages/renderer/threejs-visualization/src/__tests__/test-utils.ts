...
export function createMockStar(overrides = {}): CelestialObject {
  ...
  const base = {
    ...
    physicsStateReal: createMockPhysicsStateReal(defaultId, defaultRealMassKg, defaultPosition),
  };
...
export function createMockCelestialObject(
  ...
  const realState = createMockPhysicsStateReal(id, realMass_kg, position_m, velocity_mps);
