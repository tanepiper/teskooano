...
describe("RendererUpdater", () => {
  ...
  beforeEach(() => {
    ...
    const blackHoleRenderer =
      new SchwarzschildBlackHoleRenderer() as unknown as CelestialRenderer;
    const kerrBlackHoleRenderer =
      new KerrBlackHoleRenderer() as unknown as CelestialRenderer;
    const neutronStarRenderer =
      new NeutronStarRenderer() as unknown as CelestialRenderer;
  ...
  it("should update specialized renderers with extra parameters", () => {
    ...
    expect(specializedUpdateSpy).toHaveBeenCalledWith(
      123,
      renderer,
      scene,
      camera,
