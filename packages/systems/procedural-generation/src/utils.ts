...
export function calculateOrbitalPeriod_s(
  ...
  if (parentMass_kg <= 0 || semiMajorAxis_m <= 0) {
    console.warn(`[calculateOrbitalPeriod] Invalid input: parentMass=${parentMass_kg}, sma=${semiMajorAxis_m}. Returning 0.`);
    return 0;
  }
  const totalMass = parentMass_kg + childMass_kg;
  if (totalMass <= 0) {
      console.warn(`[calculateOrbitalPeriod] Total mass is non-positive: ${totalMass}. Returning 0.`);
      return 0;
  }
  ...
  if (termInsideSqrt < 0) {
      console.warn(`[calculateOrbitalPeriod] Term inside sqrt is negative: ${termInsideSqrt}. Returning 0.`);
      return 0;
  }
 ...
 */
export function getRandomInRange(min: number, max: number, randomFn: () => number): number {
 ...
 */
export function calculateRadius(mass_kg: number, density_kg_m3: number): number {
 ...
 */
export function calculateLuminosity(radius_m: number, temperature_k: number): number {
  if (radius_m <= 0 || temperature_k <= 0) return 0;
  const surfaceArea = 4 * Math.PI * radius_m ** 2;
  const totalPowerWatts = surfaceArea * CONST.STEFAN_BOLTZMANN * temperature_k ** 4;
...
export function mixColors(colorAHex: string, colorBHex: string, factor: number): string {
  const colorA = hexToRgb(colorAHex);
  const colorB = hexToRgb(colorBHex);

  if (!colorA || !colorB) {
    console.warn("[mixColors] Failed to parse one or both hex colors:", colorAHex, colorBHex);
 ...
 */
export function estimateTemperature(starLuminosity: number, distanceAU: number): number {
  ...
  const denominator = 16 * Math.PI * CONST.STEFAN_BOLTZMANN * (distanceMeters ** 2);
...
export function classifyGasGiantByTemperature(
    ...
    if (estimatedTemp >= CLASS_V_THRESHOLD) {
        ...
        return getRandomItem([GasGiantClass.CLASS_III, GasGiantClass.CLASS_IV], random); 
    }
}
...
export function createDetailedSurfaceProperties(
    ...
    switch (planetType) {
      ...
      default: // Fallback to BARREN procedural properties
        console.warn(`[createDetailedSurfaceProperties] Unknown planetType: ${planetType}, falling back to Barren Procedural.`);
        return {
            ...baseProps,
