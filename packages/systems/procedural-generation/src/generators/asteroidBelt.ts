...
export function generateAsteroidBelt(
  ...
  const beltType = UTIL.getRandomItem(
    [RockyType.LIGHT_ROCK, RockyType.DARK_ROCK, RockyType.ICE, RockyType.METALLIC],
    random
  );
    ...
    const beltProperties: AsteroidFieldProperties = {
        ...
        color: UTIL.getRandomItem(CONST.RING_COLORS[beltType], random).replace('c0','ff'),
    ...
    beltOrbit.period_s = UTIL.calculateOrbitalPeriod_s(starMass_kg, beltOrbit.realSemiMajorAxis_m, 0);

    // --- Validate Belt Orbit ---
    if (starMass_kg <= 0 || !Number.isFinite(starMass_kg)) {
        console.warn(`[generateAsteroidBelt] Invalid parent star mass (${starMass_kg}) for period calculation of ${beltId}. Skipping belt.`);
        return null;
    }
    if (beltOrbit.realSemiMajorAxis_m <= 0 || !Number.isFinite(beltOrbit.realSemiMajorAxis_m)) {
        console.warn(`[generateAsteroidBelt] Invalid semi-major axis (${beltOrbit.realSemiMajorAxis_m}) for ${beltId}. Skipping belt.`);
        return null;
    }
    if (beltOrbit.eccentricity < 0 || beltOrbit.eccentricity >= 1 || !Number.isFinite(beltOrbit.eccentricity)) {
        console.warn(`[generateAsteroidBelt] Invalid eccentricity (${beltOrbit.eccentricity}) for ${beltId}. Skipping belt.`);
        return null;
    }
    // --- End Validation ---
    ...
    try {
        ...
        console.warn(`[generateAsteroidBelt] Error calculating initial state for ${beltId}, using default position.`, error);
        // Fallback to a simple position at the semi-major axis distance on the x-axis
        initialPosition = new OSVector3(
  ...
  const belt: CelestialObject = {
        ...
        parentId: starId,
        realMass_kg: 0,
        realRadius_m: Math.max(beltProperties.outerRadiusAU, beltProperties.innerRadiusAU) * CONST.AU_TO_METERS,
