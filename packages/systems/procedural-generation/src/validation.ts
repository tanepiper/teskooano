...
export function validateAndCorrectHierarchy(
  ...
  console.log(
    "[validateAndCorrectHierarchy] Starting system hierarchy validation...",
  );
  ...
  const stars = correctedObjects.filter(
  ...
  const currentMainStar = stars.find(
    (star) => (star.properties as StarProperties)?.isMainStar === true,
  );
  ...
  const nonStarObjects = correctedObjects.filter(
    (obj) => obj.type !== CelestialType.STAR,
  );
  ...
  const moons = correctedObjects.filter(
  ...
  for (const moon of moons) {
    ...
    if (parent && parent.type === CelestialType.STAR) {
      ...
      const planets = correctedObjects.filter(
        (obj) =>
  ...
  const mainStar = correctedObjects.find(
    (obj) =>
  ...
  if (mainStar) {
    console.log(
      `  Main Star: ${mainStar.name} (${mainStar.realMass_kg.toExponential(2)} kg)`,
    );

    // Log companion stars
    const companions = correctedObjects.filter(
      (obj) => obj.type === CelestialType.STAR && obj.parentId === mainStar.id,
    );
    companions.forEach((star) => {
      console.log(
        `    Companion Star: ${star.name} (${star.realMass_kg.toExponential(2)} kg)`,
      );
    });

    // Log planets per star
    stars.forEach((star) => {
      const starPlanets = correctedObjects.filter(
        (obj) =>
          (obj.type === CelestialType.PLANET ||
            obj.type === CelestialType.GAS_GIANT) &&
