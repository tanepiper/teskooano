...
const consonantDoubles = ["b", "d", "f", "g", "l", "m", "n", "p", "r", "s", "t"]; 
...
export function generateCelestialName(random: () => number): string {
  ...
  for (let i = 0; i < nameLength; i++) {
    ...
    if (lastCharType === 'vowel') {
      ...
      if (allowDoubleVowel && name.length > 0 && vowelDoubles.includes(name[name.length - 1])) {
        ...
        if (nextChar === 'q' && i < nameLength - 1) { // Ensure space for 'u'
          nextChar = 'qu';
      ...
      if (allowDoubleConsonant && name.length > 0 && consonantDoubles.includes(name[name.length - 1])) {
      ...
      // First character
      if (random() < 0.6) { // Start with consonant ~60% of time
        const consonantIndex = Math.floor(random() * consonants.length);
        nextChar = consonants[consonantIndex];
