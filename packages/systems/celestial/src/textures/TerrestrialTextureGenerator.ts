...
import {
  PlanetType
} from '@teskooano/data-types'; // Corrected import path
...
export class TerrestrialTextureGenerator extends TextureGeneratorBase {
  ...
  public generateTexture(options: TerrestrialTextureOptions): TextureResult {
    ...
    return this.getOrCreateTexture(options, (opts) => this.createTerrestrialTexture(opts));
  }
   ...
   */
  private createTerrestrialTexture(options: TerrestrialTextureOptions): TextureResult {
    const { 
      type,
      surfaceColor,
      waterColor,
    ...
    const surfaceProperties = {
      ...
      planetType: type,
      color: surfaceColor instanceof THREE.Color ? surfaceColor.getHexString() : surfaceColor,
      roughness: roughness
    };
    ...
    if (type === PlanetType.OCEAN && waterColor) {
      Object.assign(surfaceProperties, {
        oceanColor: waterColor instanceof THREE.Color ? waterColor.getHexString() : waterColor
      });
    }
    ...
    if (hasVegetation && vegetationColor) {
      Object.assign(surfaceProperties, {
        vegetationColor: vegetationColor instanceof THREE.Color ? vegetationColor.getHexString() : vegetationColor
      });
    }
    ...
    if (cloudColor) {
      Object.assign(surfaceProperties, {
        cloudColor: cloudColor instanceof THREE.Color ? cloudColor.getHexString() : cloudColor,
  ...
  private mapPlanetTypeToSurfaceType(planetType: PlanetType) {
    ...
    switch (planetType) {
      case PlanetType.DESERT: return 'DUNES';
      case PlanetType.ICE: return 'ICE_FLATS';
      case PlanetType.LAVA: return 'VOLCANIC'; 
      case PlanetType.OCEAN: return 'OCEAN';
      case PlanetType.ROCKY: return 'CRATERED';
      case PlanetType.TERRESTRIAL: return 'VARIED';
      case PlanetType.BARREN: return 'CRATERED';
      default: return 'CRATERED';
    }
  }
} 