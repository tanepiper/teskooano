...
export class BaseTerrestrialRenderer implements CelestialRenderer {
  ...
  getLODLevels(object: RenderableCelestialObject, options?: CelestialMeshOptions): LODLevel[] {
    ...
    // --- Level 0: High Detail --- 
    const highDetailGroup = this._createHighDetailGroup(object, options, baseRadius);
   ...
   * @internal 
   */
  private _createHighDetailGroup(object: RenderableCelestialObject, options: CelestialMeshOptions | undefined, baseRadius: number): THREE.Group {
    const group = new THREE.Group();
    group.name = `${object.celestialObjectId}-high-lod-group`;
    const segments = options?.segments ?? (options?.detailLevel === 'high' ? 64 : 48);
    ...
    try {
        ...
        const bodyGeometry = new THREE.SphereGeometry(baseRadius, segments, segments);
        bodyMesh = new THREE.Mesh(bodyGeometry, bodyMaterial);
    } catch (error) {
        console.error(`[BaseTerrestrialRenderer] Error creating procedural material for ${object.celestialObjectId}:`, error);
        const fallbackMaterial = new THREE.MeshStandardMaterial({ color: 0x808080, roughness: 0.8, metalness: 0.1 });
        ...
        console.warn(`[BaseTerrestrialRenderer] Created fallback body mesh for ${object.celestialObjectId}:`, bodyMesh);
    }
   ...
   */
  protected createPlanetMaterial(object: RenderableCelestialObject): ProceduralPlanetMaterial {
    if (!object.celestialObjectId) {
        throw new Error("[createPlanetMaterial] CelestialObject must have an id.");
    }

    const specificSurfaceProps = (object.properties as PlanetProperties)?.surface as ProceduralSurfaceProperties | undefined;
    ...
    if (planetType) {
      switch (planetType) {
        case PlanetType.LAVA:
          simplePalette = { low: '#3B0B00', mid1: '#801800', mid2: '#D44000', high: '#FF6B00' }; // Reds/Oranges
          break;
        case PlanetType.ICE:
          simplePalette = { low: '#A0D2DB', mid1: '#C0ECF1', mid2: '#E1FEFF', high: '#FFFFFF' }; // Light blues/Whites
          break;
        case PlanetType.DESERT:
          simplePalette = { low: '#A0522D', mid1: '#D2B48C', mid2: '#E0C9A6', high: '#F5E6CA' }; // Browns/Tans
          break;
        ...
        case PlanetType.BARREN:
          simplePalette = { low: '#303030', mid1: '#606060', mid2: '#808080', high: '#B0B0B0' }; // Grayscale for rocky/barren
          break;
      }
    }
  ...
  update(
    ...
    if (!lightSources || lightSources.size === 0) {
      console.warn("[BaseTerrestrialRenderer] No light sources provided, adding default light");
      lightSources = new Map<string, { position: THREE.Vector3; color: THREE.Color; intensity: number }>();
    ...
    this.cloudMaterials.forEach(material => {
      ...
      if (camera) {
          ...
          console.warn("[BaseTerrestrialRenderer] Camera not available for cloud material update.");
  ...
  ): THREE.Mesh | null { // Return type changed
    const props = object.properties as PlanetProperties | undefined;
    const clouds = props?.clouds;
    if (!clouds) return null;
    
    const baseRadius = baseRadiusInput ?? object.realRadius_m ?? object.radius ?? 1;
  ...
  private _getBasePlanetColor(object: RenderableCelestialObject): THREE.Color {
      const planetProps = object.properties as PlanetProperties | undefined;
      const planetType = planetProps?.planetType ?? (object as any).planetType;
      const specificSurfaceProps = planetProps?.surface as ProceduralSurfaceProperties | undefined;
      ...
      if (planetType) {
          switch (planetType) {
              case PlanetType.LAVA: return new THREE.Color('#801800'); // Mid1
              case PlanetType.ICE: return new THREE.Color('#C0ECF1'); // Mid1
              case PlanetType.DESERT: return new THREE.Color('#D2B48C'); // Mid1
              case PlanetType.TERRESTRIAL: return new THREE.Color('#4C9341'); // Default Mid1 (Green)
              case PlanetType.ROCKY: 
              case PlanetType.BARREN: return new THREE.Color('#606060'); // Mid1
          }
      }
