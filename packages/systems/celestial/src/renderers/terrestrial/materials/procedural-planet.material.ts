...
export class ProceduralPlanetMaterial extends THREE.ShaderMaterial {
  ...
  constructor(surfaceProps: ProceduralSurfaceProperties) {
    ...
    const uniforms: ProceduralPlanetUniforms = {
      // Multi-light uniforms
      uNumLights: { value: 0 },
      uLightPositions: { value: Array(MAX_LIGHTS).fill(0).map(() => new THREE.Vector3()) },
      uLightColors: { value: Array(MAX_LIGHTS).fill(0).map(() => new THREE.Color(1, 1, 1)) },
    ...
    if (lightSources) {
      for (const [/*id*/, lightData] of lightSources.entries()) {
        if (numLights < MAX_LIGHTS) {
          if (lightPositions[numLights]) lightPositions[numLights].copy(lightData.position);
          if (lightColors[numLights]) lightColors[numLights].copy(lightData.color);
          if (lightIntensities) lightIntensities[numLights] = lightData.intensity; 
          numLights++;
        } else {
          break; 
        }
