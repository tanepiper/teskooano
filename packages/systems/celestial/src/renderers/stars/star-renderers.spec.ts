...
import { CelestialObject, CelestialType, StarProperties } from '@teskooano/data-types';
import { 
  createStarRenderer,
  BaseStarRenderer,
  ClassOStarRenderer,
...
describe('Star Renderers', () => {
  ...
  it('should use the correct color for each spectral class', () => {
    ...
    spectralClasses.forEach((spectralClass, index) => {
      ...
      const starBody = mesh.children.find(child => child.name === `${mockStar.id}-body`);
      expect(starBody).toBeTruthy();
      
      // Get the material and check the color (more complex in reality but simplified for test)
      const material = (starBody as THREE.Mesh).material as THREE.ShaderMaterial;
