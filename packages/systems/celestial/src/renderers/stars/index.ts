 ...
 */
export function createStarRenderer(spectralClass?: string, stellarType?: StellarType): BaseStarRenderer {
  // First check for stellar type (takes precedence)
  if (stellarType) {
    switch (stellarType) {
      case StellarType.NEUTRON_STAR: return new NeutronStarRenderer();
      case StellarType.WHITE_DWARF: return new WhiteDwarfRenderer();
      case StellarType.WOLF_RAYET: return new WolfRayetRenderer();
      case StellarType.BLACK_HOLE: return new SchwarzschildBlackHoleRenderer();
      case StellarType.KERR_BLACK_HOLE: return new KerrBlackHoleRenderer();
  ...
  switch (spectralClass?.toUpperCase()) {
    case 'O': return new ClassOStarRenderer();
    case 'B': return new ClassBStarRenderer();
    case 'A': return new ClassAStarRenderer();
    case 'F': return new ClassFStarRenderer();
    case 'G': return new ClassGStarRenderer();
    case 'K': return new ClassKStarRenderer();
    case 'M': return new ClassMStarRenderer();
    default: return new MainSequenceStarRenderer();
  }
}
