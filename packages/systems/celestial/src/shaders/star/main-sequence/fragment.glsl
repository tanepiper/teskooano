...
uniform float noiseEvolutionSpeed; // Added uniform
uniform float timeOffset;          // Added uniform
...
void main() {
    ...
    vec3 metalColor = metallicFluid(vPosition * 0.1, time); // Scale vPosition as needed
    ...
    // finalColor = max(finalColor, color * 0.75); // This line can make stars look flat, consider adjusting or removing if noise is preferred

    finalColor = mix(finalColor, vec3(1.0, 0.9, 0.7), corona_val * coronaIntensity * 0.8);
