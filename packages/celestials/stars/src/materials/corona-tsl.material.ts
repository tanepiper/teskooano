...
export class CoronaTSLMaterial extends MeshBasicNodeMaterial {
  ...
  constructor(
    ...
    const coronaEffect = Fn(() => {
      ...
      const pulse = add(
        float(1.0),
        mul(sin(mul(this.timeUniform, pulseSpeedUniform)), float(0.3)),
      );

      // Noise variation for corona turbulence
      const noisePos = mul(positionLocal, noiseScaleUniform);
      const noise = coronaNoise(
        add(noisePos, vec3(this.timeUniform, this.timeUniform, float(0.0))),
      );
    ...
    this.opacityNode = mul(
      pow(
        sub(
          float(1.0),
          abs(dot(normalize(sub(cameraPosition, positionWorld)), normalWorld)),
        ),
        float(2.0),
      ),
      this.opacityUniform,
