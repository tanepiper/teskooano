...
export class EnhancedStarTSLMaterial extends MeshStandardNodeMaterial {
  ...
  constructor(
    ...
    const plasmaTurbulenceUniform = uniform(
      float(options.plasmaTurbulence ?? 0.1),
    );
    this.lightingIntensityUniform = uniform(
      float(options.lightingIntensity ?? 1.0),
    );
    ...
    const snoise = Fn(([uvParam, resParam, timeParam]: [any, any, any]) => {
      ...
      const scaledUv = mul(
        uvParam,
        add(resParam, mul(div(timeParam, float(40000.0)), float(0.1))),
      );

      const uv0 = mul(floor(mod(scaledUv, resParam)), s);
      const uv1 = mul(
        floor(mod(add(scaledUv, vec3(1.0, 1.0, 1.0)), resParam)),
        s,
      );
      ...
      const r_next = fract(
        mul(sin(mul(add(v, sub(uv1.z, uv0.z)), float(1e-3))), float(1e5)),
      );
      const r1 = mix(
        mix(r_next.x, r_next.y, f2.x),
        mix(r_next.z, r_next.w, f2.x),
        f2.y,
      );
    ...
    const fbm = Fn(([p, time]: [any, any]) => {
      ...
      sum = add(
        sum,
        mul(snoise(mul(p, frequency), float(8.0), time), amplitude),
      );
      ...
      sum = add(
        sum,
        mul(snoise(mul(p1, frequency), float(8.0), time), amplitude),
      );
      amplitude = mul(amplitude, float(0.5));
      frequency = mul(frequency, float(2.0));
      ...
      sum = add(
        sum,
        mul(snoise(mul(p2, frequency), float(8.0), time), amplitude),
      );
    ...
    const plasmaShader = Fn(() => {
      ...
      const turbulence = mul(
        mul(fbm(turbulenceCoord, time), plasmaTurbulenceUniform),
        float(0.5),
      );

      // Combine noise effects
      const plasmaEffect = mul(
        add(plasmaNoise, turbulence),
        mul(noiseIntensityUniform, float(2.0)),
      );
      ...
      finalColor = mix(
        finalColor,
        mul(finalColor, float(1.1)),
        mul(positionVariation, float(0.2)),
      );
