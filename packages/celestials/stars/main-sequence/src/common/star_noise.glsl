#ifndef STAR_NOISE_GLSL
#define STAR_NOISE_GLSL

// From simplex/3d.glsl
vec3 mod289(vec3 x) {
...
float snoise(vec3 v) {
    ...
    vec4 j_ = p - 49.0 * floor(p * ns.z * ns.z);

    vec4 x_ = floor(j_ * ns.z);
    vec4 y_ = floor(j_ - 7.0 * x_);
