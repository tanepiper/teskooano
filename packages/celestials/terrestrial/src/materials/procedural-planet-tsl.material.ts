...
export class ProceduralPlanetTSLMaterial extends MeshStandardNodeMaterial {
  ...
  constructor(surfaceProps: ProceduralSurfaceProperties) {
    ...
    const terrainAmplitude = uniform(
      float(surfaceProps.terrainAmplitude ?? 1.0),
    );
    const terrainSharpness = uniform(
      float(surfaceProps.terrainSharpness ?? 1.0),
    );
     ...
     */
    const fbm = Fn(
      ([p, octavesParam, persistenceParam, lacunarityParam]: [
        any,
        any,
        any,
        any,
    ...
    const generateTerrain = Fn(
      ([position, typeParam, amplitudeParam, sharpnessParam, offsetParam]: [
        any,
        any,
        any,
        any,
        ...
        const ridged = sub(
          float(1.0),
          abs(mul(noiseValue, float(2.0)).sub(float(1.0))),
        );
    ...
    const terrainHeight = generateTerrain(
      objectPos,
      terrainType,
      terrainAmplitude,
      terrainSharpness,
    ...
    const undulationFactor = add(
      float(1.0),
      mul(sin(mul(terrainHeight, float(10.0))), undulation),
    );
    ...
    const perturbedNormal = normalize(
      sub(normalLocal, mul(gradient, bumpScale)),
    );
