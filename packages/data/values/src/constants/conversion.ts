/**
 * Conversion factors and unit multipliers
 *
 * Standard conversion factors for converting between different units
 ...
 *
 * Standard conversion factor for converting kilometers to meters.
 * Used throughout the simulation for distance calculations, UI display,
 * and converting between different distance units.
 *
 * @example
 * ```typescript
 ...
 * const displayDistance = distance > 1000 ? `${distance / KM} km` : `${distance} m`;
 *
 ...
 *
 * Conversion factor for converting megameters (1 million meters) to meters.
 * Used for intermediate-scale distance calculations, particularly for
 * planetary and lunar distances in the simulation.
 *
 * @example
 * ```typescript
 ...
 * const displayDistance = distance > MM ? `${distance / MM} Mm` : `${distance / KM} km`;
 * ```
 */
export const MM = 1e6;
 ...
 *
 * Conversion factor for converting gigameters (1 billion meters) to meters.
 * Used for large-scale distance calculations, particularly for
 * interplanetary distances and solar system scales.
 *
 * @example
 * ```typescript
 ...
 * const displayDistance = distance > GM ? `${distance / GM} Gm` : `${distance / MM} Mm`;
 * ```
 */
export const GM = 1e9;
 ...
 *
 * Conversion factor for converting terameters (1 trillion meters) to meters.
 * Used for very large-scale distance calculations, particularly for
 * interstellar distances and outer solar system objects.
 *
 * @example
 * ```typescript
 ...
 * const displayDistance = distance > TM ? `${distance / TM} Tm` : `${distance / GM} Gm`;
 * ```
 */
 ...
 *
 * Conversion factor for converting petameters (1 quadrillion meters) to meters.
 * Used for extremely large-scale distance calculations, particularly for
 * intergalactic distances and deep space objects.
 *
 * @example
 * ```typescript
 ...
 * const displayDistance = distance > PM ? `${distance / PM} Pm` : `${distance / TM} Tm`;
 * ```
 */
 ...
 *
 * Standard conversion factor for converting minutes to seconds.
 * Used for short time calculations, UI time displays, and
 ...
 * const displayTime = time < SECONDS_PER_HOUR ? `${time / SECONDS_PER_MINUTE} minutes` : `${time / SECONDS_PER_HOUR} hours`;
 *
 ...
 *
 * Standard conversion factor for converting hours to seconds.
 * Used for medium-term time calculations, simulation time steps,
 * and converting between different time units.
 *
 * @example
 * ```typescript
 ...
 * const displayTime = time < SECONDS_PER_DAY ? `${time / SECONDS_PER_HOUR} hours` : `${time / SECONDS_PER_DAY} days`;
 *
 ...
 *
 * Standard conversion factor for converting days to seconds.
 * Used for daily time calculations, planetary rotation periods,
 * and converting between different time units.
 *
 * @example
 * ```typescript
 ...
 * const displayTime = time < SECONDS_PER_YEAR ? `${time / SECONDS_PER_DAY} days` : `${time / SECONDS_PER_YEAR} years`;
 *
 ...
 *
 * Standard conversion factor for converting Julian years to seconds.
 * Used for annual time calculations, orbital periods, and long-term
 * time measurements in the simulation.
 *
 * @example
 * ```typescript
 ...
 * const displayTime = time > SECONDS_PER_YEAR ? `${time / SECONDS_PER_YEAR} years` : `${time / SECONDS_PER_DAY} days`;
 *
 ...
 *
 * Conversion factor for converting Gregorian years to seconds.
 * This is a simplified approximation (365 days exactly) used for
 * calendar-based calculations and UI displays where precision
 * is less critical than simplicity.
 *
 * @example
 * ```typescript
 ...
 * const displayTime = time > SECONDS_PER_YEAR_GREGORIAN ? `${time / SECONDS_PER_YEAR_GREGORIAN} years` : `${time / SECONDS_PER_DAY} days`;
 *
